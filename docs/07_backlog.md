...
### Gate: Flow 1開始前（Supabase準備）

### BL-090 Supabaseログイン・プロジェクト接続確認
...
- 関連: FR-01, FR-02
- 内容
  - Supabaseにログインし、対象プロジェクトへアクセスできることを確認
  - `apps/api/.env` と `apps/web/.env` に Supabase関連値を設定
  - Auth設定（メール/パスワード有効）を確認
- 受け入れ条件
  - API/Webの環境変数が埋まり、Flow 1（ログイン実装）着手可能な状態になる
...
- BL-001: リポジトリ骨組み作成（Done: 2026-02-15）
- BL-002: 開発ツール統一（Done: 2026-02-15）
- BL-003: 設定・シークレット管理（Done: 2026-02-15）
